...
export default function AdminDashboardPage() {
    ...
    useEffect(() => {
        ...
        // Enrollments
        const enrollmentsQuery = query(collection(firestore, "enrollments"), where("submittedAt", ">", now));
        const unsubEnrollments = onSnapshot(enrollmentsQuery, (snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === "added") {
                    const data = change.doc.data() as Enrollment;
                    toast({ title: "New Enrollment", description: `From: ${data.name}` });
                    playNotificationSound();
                }
            });
            // Update state with new data as well
            snapshot.docs.forEach(doc => {
                 const data = doc.data();
                 const submittedAt = (data.submittedAt as Timestamp)?.toDate().toLocaleString() || new Date().toLocaleString();
                 const newEntry = { id: doc.id, ...data, submittedAt, isRead: data.isRead || false } as Enrollment;
                 setEnrollments(prev => [newEntry, ...prev.filter(e => e.id !== newEntry.id)]);
            })
        });
        unsubscribes.push(unsubEnrollments);

        // Contacts
        const contactsQuery = query(collection(firestore, "contacts"), where("submittedAt", ">", now));
        const unsubContacts = onSnapshot(contactsQuery, (snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === "added") {
                    const data = change.doc.data() as ContactSubmission;
                    toast({ title: "New Contact Message", description: `From: ${data.name}` });
                    playNotificationSound();
                }
            });
            snapshot.docs.forEach(doc => {
                 const data = doc.data();
                 const submittedAt = (data.submittedAt as Timestamp)?.toDate().toLocaleString() || new Date().toLocaleString();
                 const newEntry = { id: doc.id, ...data, submittedAt } as ContactSubmission;
                 setContacts(prev => [newEntry, ...prev.filter(c => c.id !== newEntry.id)]);
            })
        });
        unsubscribes.push(unsubContacts);
        
        // Exam Registrations
        const examRegQuery = query(collection(firestore, "examRegistrations"), where("registeredAt", ">", now));
        const unsubExamReg = onSnapshot(examRegQuery, (snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === "added") {
                    const data = change.doc.data() as ExamRegistration;
                    toast({ title: "New Exam Registration", description: `By: ${data.fullName}` });
                    playNotificationSound();
                }
            });
            snapshot.docs.forEach(doc => {
                ...
                const newEntry = { id: doc.id, ...data, registeredAt, isRead: data.isRead || false } as ExamRegistration;
                setExamRegistrations(prev => [newEntry, ...prev.filter(r => r.id !== newEntry.id)]);
            });
        });
        unsubscribes.push(unsubExamReg);
        
        // Fetch initial data for contacts, enrollments, registrations
